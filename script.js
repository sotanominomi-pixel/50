document.addEventListener('DOMContentLoaded', () => {
  ...
  // Localization
  const L = {
    ja:{'tab.clock':'時計','tab.stopwatch':'ストップウォッチ','tab.alarm':'アラーム','tab.settings':'設定','btn.start':'Start','btn.stop':'Stop','btn.lap':'Lap','btn.reset':'Reset','btn.addAlarm':'アラーム追加','label.hours':'1日の長さ','footer':'設定は自動で保存されます。','msg.pickTime':'時刻を選択してください','msg.invalidTime':'不正な時刻です','msg.alarmSound':'アラームが鳴りました'},
    en:{'tab.clock':'Clock','tab.stopwatch':'Stopwatch','tab.alarm':'Alarm','tab.settings':'Settings','btn.start':'Start','btn.stop':'Stop','btn.lap':'Lap','btn.reset':'Reset','btn.addAlarm':'Add Alarm','label.hours':'Day length','footer':'Settings are saved automatically.','msg.pickTime':'Please pick a time','msg.invalidTime':'Invalid time','msg.alarmSound':'Alarm'}
  };
  ...
  function applyLocalization(){
    tabClock.textContent = t('tab.clock'); tabStopwatch.textContent = t('tab.stopwatch'); tabAlarm.textContent = t('tab.alarm'); tabSettings.textContent = t('tab.settings');
    ...
    swLap.textContent = t('btn.lap');
    swReset.textContent = t('btn.reset');
    alarmSetBtn.textContent = t('btn.addAlarm');
  }
  ...
  langSelectSettings.addEventListener('change',()=>{lang=langSelectSettings.value; applyLocalization(); saveAll();});

  // Seconds toggle
  secondsToggle.checked = showSeconds;
  secondsToggle.addEventListener('change',()=>{showSeconds=secondsToggle.checked; saveAll();});
  ...
  swStart.addEventListener('click',()=>{
    running=!running;
    if(running){ lastFrame=performance.now(); swStart.classList.replace('btn-start','btn-stop'); swLap.disabled=false; swReset.disabled=true; }
    else{ swStart.classList.replace('btn-stop','btn-start'); swLap.disabled=true; swReset.disabled=false; saveAll();}
    applyLocalization();
  });
  ...
  function playAlarmSound(){
    try{
      ...
      for(let i=0;i<6;i++){
        ...
        o.type='sine'; o.frequency.value=880-i*40;
        o.connect(gain); o.start(t0+i*0.5); o.stop(t0+i*0.5+0.35);
      }
  ...
  if(localStorage.getItem('nclock_last_triggered')) lastTriggered = localStorage.getItem('nclock_last_triggered');
  if(localStorage.getItem('nclock_show_seconds')) showSeconds = (localStorage.getItem('nclock_show_seconds')==='true');
  if(localStorage.getItem('nclock_lang')) { lang = localStorage.getItem('nclock_lang'); langSelectSettings.value=lang; }
  applyLocalization();
